...
bool KonnektingDevice::init(HardwareSerial& serial,
    ...
    pinMode(_progButton, INPUT_PULLUP);
...
int KonnektingDevice::memoryRead(int index) {
    ...
    //DEBUG_PRINTLN(F("memRead: index=0x%02x data=0x%02x"), index, d);
    return d;
}
...
void KonnektingDevice::setMemoryWriteFunc(void (*func)(int, uint8_t)) {
    eepromWriteFunc = func;
}

void KonnektingDevice::setMemoryUpdateFunc(void (*func)(int, uint8_t)) {
